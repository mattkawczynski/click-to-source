...
function createAngularFiles(tarballName, inlineTemplate = false) {
  return {
    ...
    "src/app/app.component.ts": [
      ...
      "  standalone: true,",
      inlineTemplate
        ? "  template: `\n    <main class=\"app-shell\">\n      <h1>Angular Inline Smoke Test</h1>\n      <p>Inline templates are not instrumented yet.</p>\n      <button type=\"button\" (click)=\"increment()\">Clicked {{ count() }} times</button>\n    </main>\n  `,"
...
async function main() {
  ...
  try {
    ...
    for (const spec of specs) {
      ...
      try {
        writeFiles(appRoot, createAngularFiles(tarballName, spec.inlineTemplate));
        ...
        assert(mainFile.includes('import "click-to-source/init";'), `Angular ${spec.name}: setup did not add the init import.`);
        assert(
          serve.builder === "click-to-source:dev-server",
          `Angular ${spec.name}: setup did not replace the dev-server builder.`,
        );
        assert(
          typeof serve.options === "object" && typeof serve.options.clickToSource === "object",
          `Angular ${spec.name}: setup did not add clickToSource options.`,
        );
        ...
        for (const filePath of getFiles(path.join(appRoot, "dist"))) {
          ...
          assert(
            ...
            `Angular ${spec.name}: build leaked source locations: ${filePath}`,
          );
        }
        ...
        if (spec.inlineTemplate) {
          const bundleText = await waitForText(`http://127.0.0.1:${port}/main.js`, child, logs, 120000);
          assert(
            !bundleText.includes("data-click-to-source"),
            `Angular ${spec.name}: inline templates should not be instrumented yet.\n${logs.join("")}\n${bundleText}`,
          );
          continue;
        }
        ...
        assert(
          ...
          `Angular ${spec.name}: dev bundle did not preserve button source metadata.\n${logs.join("")}`,
        );
        ...
        assert(
          ...
          `Angular ${spec.name}: open endpoint did not respond as expected.\n${logs.join("")}`,
        );
      } finally {
